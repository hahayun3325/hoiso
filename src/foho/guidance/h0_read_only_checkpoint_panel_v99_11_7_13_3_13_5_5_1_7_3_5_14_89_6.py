...
def compose_six_panel(crop_path, initial, final, object_depth, object_valid, r04, metrics_csv, output_path):
    ...
    cells = [
        _title(cell_a, 'A  input + D0: index/middle -> r04'),
        _title(cell_b, 'B  initial hand overlay'),
        _title(cell_c, 'C  accepted H0 step 5 overlay'),
        _title(cell_d, 'D  initial green / final magenta'),
        _title(cell_e, 'E  r04 cyan + selected pad vertices'),
        _title(cell_f, 'F  nonexempt depth delta + final losses'),
    ]
    canvas = Image.new('RGB', (width * 3, height * 2), color=(0, 0, 0))
    for index, image in enumerate(cells):
        canvas.paste(image, ((index % 3) * width, (index // 3) * height))
...
class ReadOnlyPanelCallback:
    ...
    def __call__(self, context):
        ...
        try:
            ...
            panel = compose_six_panel(self.crop_path, initial, final,
