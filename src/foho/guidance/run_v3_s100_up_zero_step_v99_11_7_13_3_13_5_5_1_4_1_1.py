...
from hy3dgen.shapegen.pipelines_v99_11_7_13_3_13_5_5_1_4_1_1 import (
...
def run(
    ...
    for cropped_obj_img in tqdm(assigned_imgs):
        try:
            ...
            _foho_config_for_worker = config
            import os as _foho_external_os
            if _foho_external_os.environ.get("FOHO_EXTERNAL_HAND_ANCHOR_MODE") == "object_only":
                cropped_obj_mask_path = _foho_external_os.environ["FOHO_OBJECT_MASK_PATH"]
                cropped_hand_mask_path = _foho_external_os.environ["FOHO_HAND_MASK_PATH"]
                T_h2m_path = _foho_external_os.environ["FOHO_H2M_RT_PATH"]
                _foho_base_config_value = config
                def _foho_object_only_config(*args, **kwargs):
                    _foho_cfg = _foho_base_config_value(*args, **kwargs) if callable(_foho_base_config_value) else _foho_base_config_value
                    _foho_cfg.optimization_steps_hand = 0
                    _foho_cfg.optimization_steps_scale = 5
                    _foho_cfg.optimization_steps_joint = 0
                    return _foho_cfg
                _foho_config_for_worker = _foho_object_only_config if callable(_foho_base_config_value) else _foho_object_only_config()
            obj_mesh, hand_mesh = run_hunyuan_w_guid(
                ...
                config=_foho_config_for_worker,
            )
