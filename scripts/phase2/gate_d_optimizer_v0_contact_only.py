...
out_dir = root / "gate_d_optimization/optimizer_v0_contact_only"
visual_dir = out_dir / "visuals"
metrics_path = out_dir / "gate_d_optimizer_v0_contact_only_metrics.json"
...
# Contact-only line search. Keep the cap at selected_alpha=0.75.
...
out_hand = out_dir / "hand_optimizer_v0_contact_only.ply"
optimized_hand.export(out_hand)

scene = trimesh.Scene()
scene.add_geometry(optimized_hand, node_name="hand_optimizer_v0_contact_only")
for name in region_parts:
    p = part_dir / f"{name}.ply"
    if p.exists():
        scene.add_geometry(trimesh.load(p, force="mesh", process=False), node_name=name)

out_scene = visual_dir / "scene_optimizer_v0_contact_only.glb"
scene.export(out_scene)

report = {
    "case_id": case,
    "optimizer": "gate_d_optimizer_v0_contact_only_line_search",
