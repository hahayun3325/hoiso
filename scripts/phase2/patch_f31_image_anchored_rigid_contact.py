...
Behavior is unchanged unless FOHO_F3_1_STAGE1=1. In F3 mode, the existing
...
PRELOAD_BLOCK = r'''        # FOHO_F3_1_STAGE1_BEGIN
        ...
        _foho_f3_translation_lr = float(
            os.environ.get("FOHO_F3_1_TRANSLATION_LR", "0.002")
        )
        _foho_f3_steps = int(
            os.environ.get("FOHO_F3_1_STEPS", "10")
        )
        ...
        _foho_f3_max_translation_delta_m = float(
            os.environ.get(
                "FOHO_F3_1_MAX_TRANSLATION_DELTA_M", "0.060"
            )
        )
        ...
        if _foho_f3_enabled:
            ...
            if not _foho_f3_root:
                raise RuntimeError("F3 requires FOHO_F3_1_ROOT.")

            _foho_f3_spec_path = os.path.join(
                _foho_f3_root, "inputs", "F3_1_contact_spec.json"
            )
