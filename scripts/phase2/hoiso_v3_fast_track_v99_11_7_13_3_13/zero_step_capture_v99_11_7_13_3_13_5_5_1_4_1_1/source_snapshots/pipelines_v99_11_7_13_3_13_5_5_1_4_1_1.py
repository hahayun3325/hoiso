...
from .zero_step_checkpoint_v99_11_7_13_3_13_5_5_1_4_1_1 import write_zero_step_checkpoint as _foho_write_zero_step_checkpoint
                ...
                with torch.enable_grad():
                    if _foho_f3_enabled and i == handopt_start_step:
                        ...
                        for k in range(5 if _foho_external_anchor_active else optimization_steps_scale): # optimization_steps_joint
                            ...
                            if not _foho_zero_step_saved and __import__('os').environ.get('FOHO_ZERO_STEP_OUTPUT_DIR'):
                                _foho_write_zero_step_checkpoint(
                                    __import__('os').environ.get('FOHO_ZERO_STEP_OUTPUT_DIR'),
                                    transformed_obj_mesh=transformed_obj_mesh,
                                    anchor_state=_foho_anchor_state,
                                    scale_obj=scale_obj,
                                    trans_obj=trans_obj,
                                    rotation_obj=rotation_obj,
                                    noise_pred_obj=noise_pred_obj,
                                )
                                _foho_zero_step_saved = True
                                if __import__('os').environ.get('FOHO_ZERO_STEP_CAPTURE_ONLY') == '1':
                                    break
                            total_obj_loss = (
                                1 * obj_loss +
                                10 * loss_normal + 
