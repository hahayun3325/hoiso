...
transform_name = probe.get("recommended_transform", "identity_xyz")
...
def project_vertices(v):
    v = transforms[transform_name](np.asarray(v, dtype=float))
    z = v[:, 2]
    valid_z = z > 1e-6
...
def vote_component(mesh):
    ...
    if valid.sum() < 10:
        return "discard", {
            "reason": "not_enough_visible_vertices",
    ...
    if in_obj < 0.05 and max(lid_score, base_score) < 0.05:
        return "discard", {
            "reason": "outside_all_masks",
            "in_obj": in_obj,
            "lid_score": lid_score,
...
for source_name in ["screen.ply", "keyboard_base.ply", "hinge.ply", "residual_uncertain.ply"]:
    ...
    for i, comp in enumerate(comps):
        key = f"{source_name}#component_{i:02d}"
...
decision = "PASS_RELABEL_OUTPUTS_EXIST" if result.get("lid", {}).get("exists") and result.get("base", {}).get("exists") else "FAIL_EMPTY_RELABEL_BUCKETS"

out_json = metrics_dir / "relabel_v1_report.json"
out_json.write_text(json.dumps({
    "case_id": "alapuse01",
    "stage": "safe_projection_relabel_v1",
    "transform_used": transform_name,
    "decision": decision,
    "per_component": report,
    "result": result,
    "next_step": "open relabel_v1_scene.glb if PASS; otherwise run manual component fallback"
}, indent=2))
