...
def run(
    ...
    for img_path in img_paths:
        ...
        keep_indices = np.asarray(ordered_requested[:8], dtype=np.int64)
        boxes = bboxes[keep_indices]
        right = is_right[keep_indices]
        candidate_scores = scores[keep_indices]
        ...
        for batch in dataloader:
            ...
            to_save["candidate_source_indices"] = keep_indices.copy()
            to_save["candidate_scores"] = candidate_scores.copy()
            ...
            np.save(os.path.join(out_folder, f"{img_path.stem}_same_handed_candidates_v99_11_5.npy"), to_save)
            ...
            for n in range(batch_size_local):
                ...
                if save_mesh:
                    ...
                    tmesh.export(os.path.join(out_folder, f"{img_fn.split('_')[0]}_candidate_{n:02d}_hamer.obj"))
