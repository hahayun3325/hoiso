    ...
    for img_path in img_paths:
        ...
        for vitposes in vitposes_out:
            ...
            if sum(valid) > 3:
                ...
                proposal_wrists.append(np.asarray(keyp[0, :2], dtype=np.float64))
                proposal_wrist_valid.append(bool(keyp[0, 2] > 0.3))
        ...
        bboxes = boxes
        upper_hand_roi = np.asarray([15.0, 41.0, 215.0, 251.0], dtype=np.float64)
        lower_hand_exclusion = np.asarray([287.0, 195.0, 502.0, 384.0], dtype=np.float64)
        proposal_wrists_array = np.asarray(proposal_wrists, dtype=np.float64)
        proposal_wrist_valid_array = np.asarray(proposal_wrist_valid, dtype=bool)
        bbox_centers = 0.5 * (bboxes[:, :2] + bboxes[:, 2:])
        center_inside_upper = ((bbox_centers[:, 0] >= upper_hand_roi[0]) & (bbox_centers[:, 0] <= upper_hand_roi[2]) & (bbox_centers[:, 1] >= upper_hand_roi[1]) & (bbox_centers[:, 1] <= upper_hand_roi[3]))
        wrist_inside_upper = ((proposal_wrists_array[:, 0] >= upper_hand_roi[0]) & (proposal_wrists_array[:, 0] <= upper_hand_roi[2]) & (proposal_wrists_array[:, 1] >= upper_hand_roi[1]) & (proposal_wrists_array[:, 1] <= upper_hand_roi[3]))
        center_inside_lower = ((bbox_centers[:, 0] >= lower_hand_exclusion[0]) & (bbox_centers[:, 0] <= lower_hand_exclusion[2]) & (bbox_centers[:, 1] >= lower_hand_exclusion[1]) & (bbox_centers[:, 1] <= lower_hand_exclusion[3]))
        wrist_inside_lower = ((proposal_wrists_array[:, 0] >= lower_hand_exclusion[0]) & (proposal_wrists_array[:, 0] <= lower_hand_exclusion[2]) & (proposal_wrists_array[:, 1] >= lower_hand_exclusion[1]) & (proposal_wrists_array[:, 1] <= lower_hand_exclusion[3]))
        spatial_eligible = np.where(center_inside_upper & proposal_wrist_valid_array & wrist_inside_upper & (~center_inside_lower) & (~wrist_inside_lower))[0]
        if len(spatial_eligible) == 0:
            continue
        upper_roi_center = 0.5 * (upper_hand_roi[:2] + upper_hand_roi[2:])
        upper_roi_size = np.maximum(upper_hand_roi[2:] - upper_hand_roi[:2], 1.0)
        normalized_center_distance_sq = np.sum(((bbox_centers - upper_roi_center[None, :]) / upper_roi_size[None, :]) ** 2, axis=1)
        ordered_spatial = sorted(spatial_eligible.tolist(), key=lambda source_index: (float(normalized_center_distance_sq[source_index]), int(source_index)))
        keep_indices = np.asarray(ordered_spatial[:1], dtype=np.int64)
        ...
        selected_source_wrist = proposal_wrists_array[base_source_index].copy()
        ...
        for multicrop_index, batch in enumerate(dataloader):
            ...
            to_save["upper_hand_roi_xyxy"] = upper_hand_roi.copy()
            to_save["lower_hand_exclusion_xyxy"] = lower_hand_exclusion.copy()
            to_save["roi_policy_sha256"] = '1fcac46b1cca8d1093eda1d38e0b214cb3a247d41eadf2e208a867555771b6ef'
            to_save["selected_source_index"] = int(base_source_index)
            to_save["selected_source_laterality"] = int(base_right)
            to_save["selected_source_bbox_xyxy"] = base_box.copy()
            to_save["selected_source_wrist_xy"] = selected_source_wrist.copy()
            to_save["spatial_eligibility_and_rank"] = {
                "eligible_source_indices": spatial_eligible.copy(),
                "normalized_center_distance_sq": normalized_center_distance_sq.copy(),
                "ordered_source_indices": np.asarray(ordered_spatial, dtype=np.int64),
            }
            to_save["legacy_yolo_hand_id_not_used_for_selection"] = int(yolo_hand_id)
            to_save["upper_hand_selector_version"] = "v99_11_7_9_19"
